<div align="center">

<img src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" width="60%" alt="Software Testing"/>

<h2>Hi 👋 I'm Mohamed Albayoumi</h2>
<h4>Software Tester | Manual QA | Automation & Performance Testing Learner</h4>

<!-- Social badges -->
<a href="https://www.linkedin.com/in/mohamed-elbauomy-958aa1221"
   target="_blank"
   rel="noopener noreferrer">
  <img src="https://img.shields.io/badge/LinkedIn-Mohamed%20Albayoumi-blue?style=flat&logo=linkedin">
</a>

<a href="https://mohamedalbayoumi.github.io/personal-website/"
   target="_blank"
   rel="noopener noreferrer">
  <img src="https://img.shields.io/badge/Website-Portfolio-green?style=flat&logo=google-chrome">
</a>

<br><br>

![Profile Views](https://komarev.com/ghpvc/?username=mohamedalbayoumi&style=flat&color=orange)

</div>

---

## 📫 Connect With Me

<div align="center">

<a href="mailto:mohamedelbauomy616@gmail.com"
   target="_blank"
   rel="noopener noreferrer">
  <img src="assets/gmail.svg" width="42" alt="Email"/>
</a>
&nbsp;&nbsp;&nbsp;&nbsp;
<a href="https://www.linkedin.com/in/mohamed-elbauomy-958aa1221"
   target="_blank"
   rel="noopener noreferrer">
  <img align="center"
       src="https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/linkedin.svg"
       alt="Mohamed Albayoumi LinkedIn"
       height="30"
       width="30" />
</a>


</div>

<p align="center">
📧 <strong>mohamedelbauomy616@gmail.com</strong> |
🌐 <a href="https://mohamedalbayoumi.github.io/personal-website/" target="_blank">Personal Website</a>
</p>

---

## 🧪 About Me

- 🔍 Software Tester with strong focus on **Manual Testing**
- 🧠 Good understanding of **SDLC & STLC**
- 🐞 Writing clear **test cases, test scenarios, and bug reports**
- 🤖 Currently learning **Automation Testing**
- ⚡ Exploring **Performance Testing**
- 📱 Experience testing **Web & Mobile Applications (Android & iOS)**
- 🎯 Goal: Become a **Professional QA Engineer**

---

## 🛠️ Testing Tools & Skills

### 🔹 Manual Testing
![Manual Testing](https://img.shields.io/badge/Manual%20Testing-Expert-blue)
![Test Cases](https://img.shields.io/badge/Test%20Cases-Design-green)
![Bug Reporting](https://img.shields.io/badge/Bug%20Reporting-Jira-red)
![Regression](https://img.shields.io/badge/Regression%20Testing-Yes-orange)
![UAT](https://img.shields.io/badge/UAT-Testing-purple)

### 🔹 Automation (Learning)
![Selenium](https://img.shields.io/badge/Selenium-Automation-43B02A?logo=selenium&logoColor=white)
![Python](https://img.shields.io/badge/Python-Automation-blue)
![Java](https://img.shields.io/badge/Java-Automation-orange)

### 🔹 Performance & API Testing
![JMeter](https://img.shields.io/badge/JMeter-Performance-red)
![Postman](https://img.shields.io/badge/Postman-API-orange)

### 🔹 Platforms & Tools
![Web](https://img.shields.io/badge/Web-Testing-blue)
![Android](https://img.shields.io/badge/Android-Testing-green)
![iOS](https://img.shields.io/badge/iOS-Testing-black)
![Git](https://img.shields.io/badge/Git-VersionControl-orange)
![GitHub](https://img.shields.io/badge/GitHub-Collaboration-black)
![ClickUp](https://img.shields.io/badge/ClickUp-Test%20Management-purple)
![Jira](https://img.shields.io/badge/Jira-Bug%20Tracking-blue)

---

## 🧠 QA Mindset (Code Style 😄)

```python
class SoftwareTester:
    def __init__(self):
        self.skills = [
            "Manual Testing",
            "Test Case Design",
            "Bug Reporting",
            "Regression Testing",
            "Automation Basics",
            "Performance Testing"
        ]
        self.goal = "Deliver high quality software"

    def test(self, application):
        return "Finding bugs before users do 🐞"
